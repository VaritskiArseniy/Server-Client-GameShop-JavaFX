...
public class ShowGamesUser {
    ...
    void addToBasket(MouseEvent event) throws IOException, ClassNotFoundException {
        ...
        if (games == null){
        ...
        else if (Integer.parseInt(quantityTextArea.getText()) < games.getQuantity()){
            ...
            for (Basket basket: baskets){
                if (basket.getIdGood().equals(games.getId()) && basket.getIsProgram() == 1){
            ...
            if (c==0){
                Basket basket = new Basket(null, list.get(0).getId(), games.getId(), 1, games.getGenre(), games.getName(), quantity, games.getPrice());
            ...
            JOptionPane.showMessageDialog(null, "Количество на входе больше, чем существует!");
        }
    }
    ...
    void initialize() {
        try {
            ...
            ObjectOutputStream oos = new ObjectOutputStream(socket.getOutputStream());
            ObjectInputStream ois = new ObjectInputStream(socket.getInputStream());
            Request request = new Request("GET_GAMES_AVAILABLE", 1);
            oos.writeObject(request);
            Request request1 = (Request)ois.readObject();
